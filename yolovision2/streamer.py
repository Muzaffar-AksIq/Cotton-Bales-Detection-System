...
def generate_processed_stream():
    while True:
        frame = shared_state2["last_frame"]
...
def start_flask_server2():
    def run():
        print("We are here")
        logger.info("Flask MJPEG server running at http://localhost:7863/processed")
        app.run(host="0.0.0.0", port=7863, threaded=True)
        # print("Flask Server started  processed video")
